...
from aioxcom.xcom_protocol import XcomDataMultiInfoReq, XcomDataMultiInfoReqItem, XcomDataMultiInfoRsp, XcomDataMultiInfoRspItem
...
@pytest.mark.usefixtures("context", "unused_tcp_port")
@pytest.mark.parametrize(
    "name, exp_dst_addr, exp_svc_id, exp_obj_type, exp_obj_id, exp_prop_id, rsp_flags, rsp_data, exp_except",
    [
        ("request info ok",      501, SCOM_SERVICE.READ, SCOM_OBJ_TYPE.MULTI_INFO, 0x01, SCOM_QSP_ID.NONE, 0x02, None, None),
        ("request info err",     501, SCOM_SERVICE.READ, SCOM_OBJ_TYPE.MULTI_INFO, 0x01, SCOM_QSP_ID.NONE, 0x03, XcomData.pack(SCOM_ERROR_CODES.READ_PROPERTY_FAILED, FORMAT.ERROR), XcomApiResponseIsError),
        ("request info timeout", 501, SCOM_SERVICE.READ, SCOM_OBJ_TYPE.MULTI_INFO, 0x01, SCOM_QSP_ID.NONE, 0x00, XcomData.pack(1234.0, FORMAT.FLOAT), XcomApiTimeoutException),
    ]
)
async def test_request_multi(name, exp_dst_addr, exp_svc_id, exp_obj_type, exp_obj_id, exp_prop_id, rsp_flags, rsp_data, exp_except, request):
    context = request.getfixturevalue("context")
    port    = request.getfixturevalue("unused_tcp_port")
    ...
    req_items = [
        (dataset.getByNr(3021), 0),
        (dataset.getByNr(3022), 0),
        (dataset.getByNr(3023), 0),
    ]
    exp_rsp_items = [
        XcomDataMultiInfoRspItem(3021, 0, XcomData.pack(12.3, FORMAT.FLOAT)),
        XcomDataMultiInfoRspItem(3022, 0, XcomData.pack(45.6, FORMAT.FLOAT)),
        XcomDataMultiInfoRspItem(3023, 0, XcomData.pack(78.9, FORMAT.FLOAT)),
    ]
    ...
    if exp_except == None:
        ...
        assert values.flags == exp_rsp.flags
        assert values.datetime == exp_rsp.datetime
        assert values.items != None
        assert len(values.items) == len(exp_rsp.items)

        for item in values.items:
            exp_item = next((i for i in exp_rsp.items if i.user_info_ref==item.user_info_ref and i.aggregation_type==item.aggregation_type), None)
